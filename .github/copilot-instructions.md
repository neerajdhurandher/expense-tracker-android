...
- Methods: `signIn(email, displayName, photoUrl?) → Result<User>`, `signOut() → Result<Unit>`
...
- **Composable:** `HomeScreen(viewModel, userName, userEmail, onNavigateToGraph, onNavigateToCategories, onSignOut)`
- **Sections:**
    1. Header (title + avatar + profile dropdown menu)
...
- **Test tags:** `add_expense_fab`, `profile_avatar`, `menu_manage_categories`, `menu_sign_out`, `view_graph_button`, `total_amount_text`, `untracked_section_header`, `filter_button`, `expense_item_{id}`, `untracked_item_{id}`, `untracked_confirm_{id}`, `untracked_edit_{id}`, `untracked_dismiss_{id}`
